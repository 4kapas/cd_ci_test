...
export class TextSprite extends THREE.Object3D {
  ...
  update(type) {
    if (type === "edit") {
      ...
      if (!this.text) {
        console.error("TextSprite.update: 'this.text' is undefined or null.");
        this.text = ""; // 기본 값을 설정하거나 다른 처리를 수행
      }

      let textSegments = this.text.split("&");

      let maxTextWidth = 0;
      textSegments.forEach((segment) => {
        let metrics = context.measureText(segment);
        maxTextWidth = Math.max(maxTextWidth, metrics.width);
      });

      let margin = 5;
      let spriteWidth = 2 * margin + maxTextWidth + 20 * this.borderThickness;
      let spriteHeight =
        (this.fontsize * 1.4 + 2 * this.borderThickness) * textSegments.length;
      ...
      this.roundRect(
        ...
        this.borderThickness / 2,
        spriteWidth - this.borderThickness,
        spriteHeight - this.borderThickness,
        6
      );

      let y = this.fontsize + this.borderThickness;
      textSegments.forEach((segment) => {
        context.fillStyle = "rgba(10, 5, 2, 1.0)"; // 텍스트 색상을 검은색으로 설정
        context.fillText(segment.trim(), this.borderThickness + margin, y);
        y += this.fontsize * 1.4;
      });
