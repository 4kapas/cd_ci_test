	...
	onMouseDown(e) {
		...
		if (this.hoveredElements.length === 0) {
			...
			for (let hovered of this.hoveredElements) {
				...
				// Dorosee Custom 23/09/?? 작업자 : 양태현
				if(object.parent instanceof Measure){
					let parent = object.parent;
					let measurementsRoot = $("#jstree_scene").jstree().get_json("measurements");
					let jsonNode = measurementsRoot.children.find(child => child.data.uuid === parent.uuid);
					$.jstree.reference(jsonNode.id).deselect_all();
					$.jstree.reference(jsonNode.id).select_node(jsonNode.id);
				}
				if(object.parent.isShp){
					object.material.color.set(0xff0000); // Material의 색상 변경
				}
				//
