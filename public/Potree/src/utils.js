...
export class Utils {
  static async loadShapefileFeatures(shp, dbf, callback) {
    console.log(dbf, "objpath");
    let features = [];
    ...
    let source;
    if (dbf) {
      source = await shapefile.open(shp, dbf, { encoding: "euc-kr" });
    } else {
      console.log(dbf, "objpath to shp");
  ...
  // Dorosee Custom 23/04/07
  static async loadShapefileFeaturesShpDbf(shp, dbf, callback) {
    let features = [];

    let handleFinish = () => {
      callback(features);
    };
    let source = await shapefile.open(shp, dbf, { encoding: "euc-kr" });
    while (true) {
      let result = await source.read();
