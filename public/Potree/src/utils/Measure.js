...
function createAreaLabel() {
  ...
  areaLabel.setBorderColor({ r: 0, g: 0, b: 0, a: 0 });
  areaLabel.setBackgroundColor({ r: 0, g: 0, b: 0, a: 0 });
  areaLabel.fontsize = 28;
...
export class Measure extends THREE.Object3D {
  constructor(type, data) {
    ...
    this.initialAreaCalculated = false;
    this.initialArea = 0;
  ...
  calculateInitialArea() {
    if (this.initialAreaCalculated == false) {
      // 면적 계산 로직...
      console.log("들어왓니?");
      let area = 0;
      ...
      this.initialArea = Math.abs(area / 2);
    }
    this.initialAreaCalculated = true;
  }
  ...
  addMarker(point) {
...

    {
      ...
      // let clickEvent = (e) => {
      //   console.log(e.target.position, "클릭");
      // };
      ...
      // sphere.addEventListener("click", clickEvent, false);
      // 클릭 이벤트 리스너 추가
      sphere.addEventListener("click", (event) => {
        const sphere = event.target; // 클릭된 구체 객체
        const position = sphere.position; // 구체의 위치 정보를 복제하여 사용
  ...
  updatePolygonArea() {
    ...
    // ShapeGeometry를 사용하여 폴리곤 지오메트리 생성
    const geometry = new THREE.ShapeGeometry(shape);

    // Material 생성
    const material = new THREE.MeshBasicMaterial({
      color: 0xff0000, // 색상
      side: THREE.DoubleSide, // 양면 보이도록 설정
    ...
    this.polygonMesh.position.z = 15;
  ...
  update(type) {
    if (this.points.length === 0) {
      ...
      if (coordinateLabel.parent.name === "Point") {
        let posArray = position.toArray();

        // position에서 x, y, z의 순서를 y, x, z로 변경
        position = [posArray[1], posArray[0], posArray[2]]; // y, x, z 순서로 재배열

        let msg = position
          .map((p) => Utils.addCommas(p.toFixed(3)))
          .join(" / ");
...

    {
      ...
      if (
        this.type === "editMeasure" &&
        this.data?.properties?.REGIAREA !== undefined
      ) {
        const areaRegi = parseFloat(this.data?.properties?.REGIAREA);
        const areaText = parseFloat(txtArea.replace(/,/g, "")); // 콤마 제거 후 변환
        const areaDifference = areaText - areaRegi;

        // 오차에 따른 부호 설정
        let errorDisplay;
        if (areaDifference === 0) {
          errorDisplay = "0"; // 오차가 0일 경우
        } else {
          const sign = areaDifference > 0 ? "+" : "-";
          errorDisplay = `${sign}${Math.abs(areaDifference).toFixed(1)}`; // 오차가 0이 아닐 경우, 부호와 함께 표시
        }

        // 최종 메시지 구성
        msg = `${areaRegi}${suffix}²&${txtArea}${suffix}²&${errorDisplay}`;

        this.data.properties.editAreaData = areaText;
        this.data.properties.editAreaMistake = Math.abs(areaDifference);
        this.areaLabel.setText(msg, "edit");
      } else {
        msg = `${txtArea}${suffix}²`;
        this.areaLabel.setText(msg);
      }
      this.areaLabel.fontsize = 28;
    }
