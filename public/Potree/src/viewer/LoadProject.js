...
function loadMeasurement(viewer, data) {
  ...
  console.log(data, "loadmesirement");
  if (duplicate) {
  ...
  if (!data.isEditArea) {
    const measure = new Measure("editMeasure", data.userData);
    ...
    measure.closed = true;
    measure.isEditArea = data.isEditArea;
    measure.initialPoints = data.initialPoints;
    measure.userData = data.userData;
    ...
    measure.userData = data.userData;
    measure.areaLabel.material.color = { r: 236, g: 0, b: 213 };
...
function loadAnnotations(viewer, data) {
  console.log(viewer, "anotaions viewer");
  if (!data) {
    return;
  }
...
/**
 * 23/03/29 추가 포인트클라우드를 로드하고싶지않아서 만든 함수
 * @param {*} viewer
 * @param {*} data
 * @returns
 */
export async function loadProjectDorosee(viewer, data) {
  if (data.type !== "Potree") {
    console.error("not a valid Potree project");
    return;
  }
  ...
  loadView(viewer, data.view);
  let loadMesasureUserData = [];

  for (const measure of data.measurements) {
    let result = loadMeasurement(viewer, measure);
    loadMesasureUserData.push(result);
  }
  ...
  return loadMesasureUserData;
}

export async function loadUrlVolumBoxDorosee(viewer, data) {
  console.log(data, "여기야?");

  loadVolume(viewer, data);
}
