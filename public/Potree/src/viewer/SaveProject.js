...
function createMeasurementData(measurement) {
  console.log(measurement, "save mesdflksdlfkj");
  const toArray = (color) => [color.r, color.g, color.b];
  const data = {
    ...
    isEditArea: measurement.isEditArea,
    userData: measurement.userData,
    initialPoints: measurement.initialPoints,
    color: measurement.color.toArray
      ? measurement.color.toArray()
      : toArray(measurement.color),
  };
...
function createAnnotationsData(viewer) {
  ...
  const annotations = viewer?.scene?.annotations.children.map((a) =>
    map.get(a)
  );
...
function createSettingsData(viewer, number) {
  return {
    pointBudget: number == 2 ? 1000000 : viewer?.getPointBudget(),
    fov: viewer?.getFOV(),
...
export function saveProject(viewer, number) {
  console.log(viewer, viewer?.scene, "viewer scene?");
  const scene = viewer?.scene;

  const data = {
    type: "Potree",
    version: 1.7,
    settings: createSettingsData(viewer, number),
