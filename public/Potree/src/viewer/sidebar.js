...
export class Sidebar {
  ...
  initScene() {
    ...
    tree.on("select_node.jstree", (e, data) => {
      ...
      // Dorosee Custom 23.09.?? 추가한사람 : 양태현
      if (object.isShp) {
        console.log("shp object", object);
      }
      if (object instanceof Measure) {
        let allMeasurements = this.viewer.scene.measurements;
        // console.log({ allMeasurements })
        for (let measurement of allMeasurements) {
          if (measurement.name !== "Area") continue;
          if (measurement === object) {
            object.traverse((child) => {
              if (child.type === "Line2") {
                child.material.color.set(0xff0000); // 모든 Mesh 객체를 빨간색으로 변경
              }
            });
          } else {
            measurement.traverse((child) => {
              if (child.type === "Line2") {
                child.material.color.set(0xffff00); // 모든 Mesh 객체를 노란색으로 변경
              }
            });
          }
        }
    ...
    tree.on("check_node.jstree uncheck_node.jstree", function (e, data) {
      ...
      if (parentNode) {
        // 부모 노드가 있는 경우 (루트 노드가 아닌 경우)
        if (parentNode.children.length > 0) {
          let state = parentNode.state.checked ? "check_node" : "uncheck_node";
          // 자식 노드의 체크 상태를 부모 노드와 동기화
          for (let children of parentNode.children) {
            tree.jstree(state, children);
          }

          // data.instance.set_state(parentNode.children, { checked: parentNode.state.checked });
        }
      }
    });
  ...
  initAppearance() {
    ...
    $("#sldEDLRadius").slider({
      ...
      range: "max",
      slide: (event, ui) => {
        $("#lblEDLRadius").val(Math.floor(ui.value));
        this.viewer.setEDLRadius(Math.floor(ui.value));
      },
    });
    ...
    $("#sldEDLOpacity").slider({
      ...
      range: "max",
      slide: (event, ui) => {
        $("#lblOpacity").val(ui.value);
        this.viewer.setEDLOpacity(ui.value);
      },
    });

    $("#amountlblOpacity").val($("#sldEDLOpacity").slider("value"));
    ...
    this.viewer.addEventListener("edl_radius_changed", (event) => {
      $("#lblEDLRadius")[0].innerHTML = this.viewer.getEDLRadius();
