...
export class Viewer extends EventDispatcher {
  constructor(domElement, args = {}) {
    ...
    try {
      {
        console.log(domElement.id, "domel");
        // generate missing dom hierarchy
        if ($(domElement).find("#potree_map").length === 0) {
          let potreeMap = $(`
            <div id="potree_map" class="mapBox" style="position: absolute; right: 50px; bottom: 50px; width: 400px; height: 400px; display: none">
  ...
  setScene(scene) {
...

    {
      ...
      console.log(this.renderArea.id, "this.renderArea");
      if (!this.onAnnotationAdded) {
        this.onAnnotationAdded = (e) => {
          // console.log("annotation added: " + e.annotation.title);

          e.annotation.traverse((node) => {
            console.log(node.domElement, "annotation.domElement");

            $(`#${this.renderArea.id} #potree_annotation_container`).append(
              node.domElement
            );
  ...
  //dorosee
  async loadProjectByText(text, view, viewNumber) {
    const json = JSON5.parse(text);
    // console.log("loadProjectByText", text);
    let loadProjectDorosee;
    if (json.type === "Potree") {
      loadProjectDorosee = Potree.loadProjectDorosee(view, json, viewNumber);
    }
    return loadProjectDorosee;
    //Potree.loadProject(this, url);
  }

  async loadUrlVolumBox(view, data) {
    console.log(data, "volumesData통과?");

    Potree.loadUrlVolumBoxDorosee(view, data);
  }
  ...
  toggleSidebar() {
    ...
    if (isVisible) {
      ...
      //orginal
      renderArea.css("left", "294px");

      // // dorosee yang : 2023/03/07
      // renderArea.css('left', '675px');
    }
  }
    ...
    if (shpState) {
      ...
      sidebarContainer.load(
        ...
        () => {
          ...
          let imgMapToggle = document.getElementById("potree_map_toggle");
          // imgMapToggle.src = new URL(
          //   Potree.resourcePath + "/icons/map_icon.svg"
          // ).href;
          imgMapToggle.style.position = "fixed";
          imgMapToggle.onclick = (e) => {
            this.toggleMap();
            // const currentPosition = imgMapToggle.style.position;

            // if (currentPosition === "fixed") {
            //   imgMapToggle.style.display = "none";
            //   imgMapToggle.style.position = "absolute";
            // } else {
            //   imgMapToggle.style.position = "fixed";
            //   imgMapToggle.style.display = "block";
            // }
          };
          ...
          $(() => {
            ...
            sidebar.init(shpState);
            console.log("cojalksn", sidebar);
            ...
            let elProfile = $("<div>").load(
              ...
              () => {
                ...
                this.profileWindowController = new ProfileWindowController(
                  this
                );
                ...
                $("#prop_info_window").draggable(); //속성정보
                $("#layer_window").draggable({
                  cancel: "#annotation_title, #annotation_description",
                }); //레이어

                $("#setting_window").draggable(); //설정
                $("#setting_window").draggable({
                  cancel:
                    ".notDraged span, .notDraged input, .notDraged shpMoveArrows",
                });
  ...
  update(delta, timestamp) {
    ...
    if (this.mapView) {
      ...
      if (this.mapView.sceneProjection) {
        $("#potree_map_toggle").css("display", "fixed");
      }
    }
  ...
  renderDefault() {
    ...
    // console.log("pRenderer",pRenderer);
    {
      // resize
